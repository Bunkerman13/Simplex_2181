...
namespace Simplex
{
	...
	class A06Octant
	{
		...
		A06Octant* m_pParent = nullptr;// Will store the parent of current octant
		A06Octant* m_pChild[8];//Will store the children of the current octant

		std::vector<uint> m_EntityList; //List of Entities under this octant (Index in Entity Manager)

		A06Octant* m_pRoot = nullptr;//Root octant
		std::vector<A06Octant*> m_lChild; //list of nodes that contain objects (this will be applied to root only)
	...
	public:
		...
		OUTPUT: class object
		*/
		A06Octant(uint a_nMaxLevel = 2, uint a_nIdealEntityCount = 5);
		/*
		USAGE: Constructor
		ARGUMENTS:
		...
		OUTPUT: class object
		*/
		A06Octant(vector3 a_v3Center, float a_fSize);
		/*
		USAGE: Copy Constructor
		ARGUMENTS: class object to copy
		OUTPUT: class object instance
		*/
		A06Octant(A06Octant const& other);
		/*
		USAGE: Copy Assignment Operator
		ARGUMENTS: class object to copy
		OUTPUT: ---
		*/
		A06Octant& operator=(A06Octant const& other);
		/*
		USAGE: Destructor
		ARGUMENTS: ---
		OUTPUT: ---
